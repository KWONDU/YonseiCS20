...
def writeText():
  '''
  :return: return nothing, just display mouse's coordinate at the lower-left corner
  '''
  x_cor = int(t.xcor()); y_cor = int(t.ycor()) # get mouse's coordinate
  cor_t.clear() # erase the previous text
  cor_t.setpos(0, 0) # set position to the lower-left corner
  cor_t.write('(' + str(x_cor) + ', ' + str(y_cor) + ')',
          font = ('Times New Roman', 8, 'bold')) # display mouse's coordinate
...
# declare new turtle cor_t, which uses to display mouse's coordinate
cor_t = turtle.Turtle()
cor_t.penup()
