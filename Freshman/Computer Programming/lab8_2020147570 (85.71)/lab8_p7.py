...
# init wins of horses
Horse_won = [0 for i in range(num_horses)]
# init user's input
chk = ''

# if user pressess 'q', stop the program
while chk != 'q':
    # clear the screen
    window.clear()
    # hide default turtle and keep from drawing - init turtle when loop starts
    turtle.hideturtle()
    turtle.penup()
    ...
    # increase winning horse's cumulative wins by 1
    Horse_won[winner] = Horse_won[winner] + 1
    # set user's input to chk
    chk = input('Press \'q\' to quit: ')

# close the graphics window
turtle.bye()
# display the cumulative wins of all horses
for  i in range(num_horses):
    print('Horse', i, 'won', Horse_won[i], 'times')

# terminate program when close window
# turtle.exitonclick() - is replaced by turtle.bye()
